...
def send_to_influxdb(url, spo2, bpm, pi, alarm, exc):
    ...
    data = f"SPO2 value={clean_value(spo2)}\nBPM value={clean_value(bpm)}\nPI value={clean_value(pi)}\nALARM value={alarm}\nEXC value={exc}"

    try:
        response = requests.post(url, data=data, headers={'Content-Type': 'text/plain'})
        if response.status_code != 204:
            logging.warning(f"[WARN] InfluxDB Antwort: {response.status_code} - {response.text}")
        else:
            logging.debug("[DATA] Werte erfolgreich an InfluxDB gesendet")
    except Exception as e:
        logging.error(f"[ERROR] Fehler beim Senden an InfluxDB: {e}")
...
def main():
    ...
    influx_url = config_raw['output'].get('influx_url')
            ...
            with ser, open(output_path, 'a', buffering=1) as outfile:
                ...
                while True:
                    ...
                    if line:
                        ...
                        if influx_url:
                            send_to_influxdb(influx_url, spo2, bpm, pi, alarm, exc)
